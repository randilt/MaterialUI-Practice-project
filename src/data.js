export const clothingProducts = [
  {
    name: "T-Shirt",
    price: 19.99,
    imageUrl: "https://example.com/tshirt.jpg",
  },
  {
    name: "Jeans",
    price: 39.99,
    imageUrl: "https://example.com/jeans.jpg",
  },
  {
    name: "Dress Shirt",
    price: 29.99,
    imageUrl: "https://example.com/dress_shirt.jpg",
  },
  {
    name: "Skirt",
    price: 24.99,
    imageUrl: "https://example.com/skirt.jpg",
  },
  {
    name: "Hoodie",
    price: 49.99,
    imageUrl: "https://example.com/hoodie.jpg",
  },
  {
    name: "Dress",
    price: 59.99,
    imageUrl: "https://example.com/dress.jpg",
  },
  {
    name: "Sweatpants",
    price: 34.99,
    imageUrl: "https://example.com/sweatpants.jpg",
  },
  {
    name: "Jacket",
    price: 79.99,
    imageUrl: "https://example.com/jacket.jpg",
  },
  {
    name: "Blouse",
    price: 27.99,
    imageUrl: "https://example.com/blouse.jpg",
  },
  {
    name: "Shorts",
    price: 22.99,
    imageUrl: "https://example.com/shorts.jpg",
  },
  {
    name: "Sweater",
    price: 44.99,
    imageUrl: "https://example.com/sweater.jpg",
  },
  {
    name: "Polo Shirt",
    price: 34.99,
    imageUrl: "https://example.com/polo_shirt.jpg",
  },
  {
    name: "Coat",
    price: 89.99,
    imageUrl: "https://example.com/coat.jpg",
  },
  {
    name: "Leggings",
    price: 29.99,
    imageUrl: "https://example.com/leggings.jpg",
  },
  {
    name: "Tank Top",
    price: 14.99,
    imageUrl: "https://example.com/tank_top.jpg",
  },
];

export const posts = [
  {
    username: "John Doe",
    img: "https://www.shutterstock.com/image-photo/majestic-view-on-turquoise-water-600nw-266538056.jpg",
    date: "September 14, 2016",
    content:
      "This impressive paella is a perfect party dish and a fun meal to cook together with your guests. Add 1 cup of frozen peas along with the mussels, if you like.",
  },
  {
    username: "Jane Smith",
    img: "https://i.ytimg.com/vi/pG4wktJyGnE/maxresdefault.jpg",
    date: "October 24, 2016",
    content:
      "Capturing the Fluid Beauty: Witness the Cascade of Creativity Crafted by Artificial Intelligence, Where Nature's Elegance and Technological Ingenuity Converge in a Mesmerizing Symphony of Water and Artistry. 🌊✨",
  },
  {
    username: "Max Mustermann",
    img: "https://www.leetdesk.com/_next/image?url=https%3A%2F%2Fimages.prismic.io%2Fleetdesk%2Fc5527f47-def9-433c-9cce-23f286dbfea2_Front.jpg%3Fauto%3Dcompress%2Cformat%26rect%3D0%2C374%2C4000%2C2250%26w%3D1920%26h%3D1080&w=3840&q=75",
    date: "November 11, 2016",
    content:
      "Immersive Workstation: Dive into Productivity with a Triple Monitor Setup, Where Screens Unite to Amplify Efficiency and Creativity in the Digital Realm, Blending Technology and Functionality in Harmonious Brilliance. 💻🖥️🌟",
  },
  {
    username: "Anna Schmidt",
    img: "https://www.krugerpark.co.za/images/spekes-hinged-back-tortoise-south-africa-786x416.jpg",
    date: "December 31, 2016",
    content:
      "Nature's Wonders: Behold the Majestic Beauty of the Speke's Hinged-Back Tortoise, an Endearing Emblem of Resilience and Grace, Roaming the Vast Plains of South Africa in a Timeless Dance of Life and Harmony. 🐢🌍✨",
  },
];
